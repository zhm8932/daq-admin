...
(function($) {"use strict";
	...
	Grid.prototype = {
		...
		_initLayout : function() {
			...
			//有待考证
			this.$element.find('.grid-table-body').css('height', '600px');
		},
			...
			for (var i = 0, j = items.length; i < j; i++) {
				...
				for (var k = 0, h = this.options.columns.length; k < h; k++) {
					...
					if (self.options.tree && self.options.tree.column && self.options.tree.column == column.name) {
						trHtml.push('<div class="grid-tree-space" style="padding-left:' + (parseInt(item.level-originLevel)) * 25 + 'px;"><span data-role="grid-tree-icon" class="glyphicon glyphicon-folder-open open"></span></div>&nbsp;&nbsp;');
					}
					if (column.render) {
		 ...
		 * 初始化树形数据
		 */
		initTreeItems : function(newItems, items,parentName) {
			var self = this;
			for (var i = 0, j = items.length; i < j; i++) {
				var item = items[i];
				...
				newItems.push(item);
				if (item.children) {
					newItems = self.initTreeItems(newItems, item.children,item.name);
				}
			}
			return newItems;
		},
